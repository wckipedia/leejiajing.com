	...
	const technologies = [
		{
			...
			iconBg: 'rgba(255, 62, 0, 0.14)'
		},
		{
			name: 'TypeScript',
			description: 'JavaScript but better',
			icon: si('typescript', '3178C6'),
			iconBg: 'rgba(49, 120, 198, 0.14)'
		},
		{
			name: 'Python',
			description: 'General-purpose language',
			icon: si('python', '3776AB'),
			iconBg: 'rgba(55, 118, 171, 0.14)'
		},
		{
			name: 'C#',
		...
		},
		{
			...
			iconBg: 'rgba(68, 121, 161, 0.14)'
		},
		{
			name: 'MongoDB',
			description: 'NoSQL database',
			icon: si('mongodb', '47A248'),
			iconBg: 'rgba(71, 162, 72, 0.14)'
		},
		{
			name: 'AWS',
			...
			iconBg: 'rgba(255, 153, 0, 0.16)'
		},
		{
			name: 'Git',
			description: 'Version control',
			icon: si('git', 'F05032'),
			iconBg: 'rgba(240, 80, 50, 0.14)'
		}
	];
...
<section id="skills" class="mx-auto max-w-6xl scroll-mt-[4.5rem] px-4 py-12 sm:scroll-mt-20 sm:px-6 sm:py-16 md:scroll-mt-24">
	...
	<ul
		class="mt-8 grid gap-3 sm:gap-4 [grid-template-columns:repeat(auto-fit,minmax(min(100%,15.625rem),1fr))]"
	>
		{#each technologies as technology (technology.name)}
			<li
				class="flex items-center gap-3.5 rounded-xl border border-stone-300/80 bg-[#fbf7ef] p-4 shadow-sm transition hover:border-stone-400 hover:bg-white hover:shadow-md sm:gap-4 sm:p-5"
			>
				<div
					class="flex size-10 shrink-0 items-center justify-center rounded-lg sm:size-11"
					style={`background-color: ${technology.iconBg};`}
				>
					<img
				...
				<div class="min-w-0">
					<p class="text-sm font-bold tracking-[-0.02em] text-stone-950 sm:text-base">
						{technology.name}
					</p>
					<p class="mt-0.5 text-xs leading-snug text-stone-600 sm:text-sm">
